...
describe('RMRKEmotableRepositoryMock', async function () {
  ...
  it('can support IERC721', async function () {
    expect(await repository.supportsInterface('0x80ac58cd')).to.equal(true);
  });
  ...
  describe('With minted tokens', async function () {
    ...
    it('can emote', async function () {
      ...
      expect(await repository.emoteCoutOf(token.address, tokenId, emoji1)).to.equal(bn(1));
    });
    ...
    it('can undo emote', async function () {
      ...
      expect(await repository.emoteCoutOf(token.address, tokenId, emoji1)).to.equal(bn(0));
    });
    ...
    it('can be emoted from different accounts', async function () {
      ...
      expect(await repository.emoteCoutOf(token.address, tokenId, emoji1)).to.equal(bn(2));
      expect(await repository.emoteCoutOf(token.address, tokenId, emoji2)).to.equal(bn(1));
    });
    ...
    it('does nothing if new state is the same as old state', async function () {
      ...
      expect(await repository.emoteCoutOf(token.address, tokenId, emoji2)).to.equal(bn(0));
    });
  });
});
